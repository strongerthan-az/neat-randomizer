...
function generateIdea() {
  ...
  currentTask = options[Math.floor(Math.random() * options.length)];
  taskCompleted = false;
  document.getElementById('ideaBox').innerText = currentTask;
}

function completeIdea() {
  if (!currentTask) {
    alert("Please generate a NEAT idea first!");
    return;
  }

  if (taskCompleted) {
    alert("You've already completed this task. Generate a new one to earn more XP!");
    return;
  }
